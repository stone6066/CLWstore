...
- (void)updateUserPhotos:(NSNotification *) noti
{
    ...
    if ([maPhotos isKindOfClass:[NSArray class]] && maPhotos.count > 0) {
        ...
        UIImage *image_main = [self AssetsToImage:asset];
        _iv_main.image = image_main;
        _lbl_upload.alpha = 0;
    } else {
        index_main_image = -1 ; //这里其实已经变成没有了
...
- (UIImage *)AssetsToImage:(ZLPhotoAssets *)asset {
    
    //ZLPhotoAssets *asset = maPhotos[index_main_image];
    UIImage *image_main = nil;
...
- (void) getBrandInfoFromNetwork
{
    ...
    NSArray *arr  = [NSArray arrayWithContentsOfFile:dataFilePath];
...
- (void) getAccessoryColorInfoFromNetwork
{
    ...
    NSString *dataFilePath = [DocumentBasePath stringByAppendingFormat:@"/%@.plist", @"Colour"];
    NSArray *arr  = [NSArray arrayWithContentsOfFile:dataFilePath];
...
- (void) getAccessoryPurityInfoFromNetwork
{
    ...
    NSString *dataFilePath = [DocumentBasePath stringByAppendingFormat:@"/%@.plist", @"Purity"];
    NSArray *arr  = [NSArray arrayWithContentsOfFile:dataFilePath];
...
- (void) getAccessorySourceInfoFromNetwork
{
    ...
    NSString *dataFilePath = [DocumentBasePath stringByAppendingFormat:@"/%@.plist", @"PartsSrc"];
    NSArray *arr  = [NSArray arrayWithContentsOfFile:dataFilePath];
...
- (void) postAccessoryInfosToNetwork
{
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@%@",BASEURL,k_url_UsrStore_AddParts];
    ...
    NSDictionary *paramDic1 = @{
                                ...
                                @"Spec":@"",
                                @"ColourId": currentCarAccessoryColor.Id,//@"358c7cd8-9eba-4516-b888-86ae9ec15cdc",
    ...
    NSArray *params = @[paramDic2];
    
    NSDictionary *jsonParam = @{
                                @"partJson":[JYJSON JSONStringWithDictionaryOrArray:paramDic1],
                                @"carModelJson":[JYJSON JSONStringWithDictionaryOrArray:params]
                                };
    
    NSString *json = [JYJSON JSONStringWithDictionaryOrArray:jsonParam];
    NSLog(@"json = %@", json);
        ...
        if (task.state == NSURLSessionTaskStateCompleted) {
            ...
            if ([status isEqualToString:@"1"]) {
                ...
                if (index_main_image != -1) {
                    ...
                    for (int i=0; i<maPhotos.count; i++) {
                        
                        BOOL isFirst = false;
                        if (index_main_image == i) {
                            
                            isFirst = true;
                        }
                        
                        UIImage *image = [self AssetsToImage:maPhotos[i]];
                        
                        [self postAccessoryImagesToNetwork:image partsid:uuid isFirst:isFirst isSingle:isSingle];
                    }
 ...
 *  上传配件图片
 */
- (void)postAccessoryImagesToNetwork:(UIImage *)toSendImage partsid:(NSString *)partsid isFirst:(BOOL)isFirst isSingle:(BOOL)isSingle
{
    ...
    if (isSingle) {
        ...
        if (isFirst) {
            ...
            paramDict = @{
                          ...
                          @"dataId":partsid,  //ApplicationDelegate.userInfo.user_Id,//必须是用户的id
                          @"isSingle":@"true",
                          @"isFirst":@"true"
                          };
...
//                                };
    ...
    [ApplicationDelegate.httpManager POST:urlStr parameters:paramDict constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        NSData *imageData = UIImageJPEGRepresentation(toSendImage, 0.3);
        [formData appendPartWithFileData:imageData name:ApplicationDelegate.userInfo.user_Id fileName:ApplicationDelegate.userInfo.user_Id mimeType:@"image/jpg"];
        
        //NSData *imageData = UIImagePNGRepresentation(self.iv_main.image);
